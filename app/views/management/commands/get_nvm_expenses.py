...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for x in range(1991,2024):
            years += [str(x)]
        expense_nvm = pd.read_excel('https://www.transit.dot.gov/sites/fta.dot.gov/files/2024-10/2023%20TS2.1%20Service%20Data%20and%20Operating%20Expenses%20Time%20Series%20by%20Mode.xlsx', sheet_name="OpExp NVM", engine="openpyxl")
        expense_nvm[years] = expense_nvm[years].fillna(0)
        expense_nvm[['UACE Code', 'UZA Area SQ Miles', 'UZA Population', 'NTD ID']] = expense_nvm[['UACE Code', 'UZA Area SQ Miles', 'UZA Population', 'NTD ID']].fillna(0)
        for x in expense_nvm.index: 
            ...
            if len(transit_agencies) < 1:
                transit_agency = TransitAgency(
                    ...
                    uza_name = expense_nvm['Primary UZA Name'][x],
                    uza = expense_nvm['UACE Code'][x],
                    uza_area_sqm = expense_nvm['UZA Area SQ Miles'][x],
                    uza_population = expense_nvm['UZA Population'][x],
                    # status_2021 = expense_nvm['Agency Status'][x],
                )
                transit_agency.save()
            else:
