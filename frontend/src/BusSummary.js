...
function BusSummary(props) {
  ...
  React.useEffect(() => {
    if (props.params) {
      // axios.get(`http://45.33.31.186/spending_by_budget/?mode=MB,CB,RB,TB,PB${props.params}`)
      ...
      axios.get(`http://45.33.31.186/cost_per_pmt_by_mode/?mode=MB,CB,RB,TB,PB${props.params}`)
        ...
        axios.get(`http://45.33.31.186/spending_by_mode/?mode=MB,CB,RB,TB,PB${props.params}&expense_type=VO,VM,NVM,GA`)
          .then(response => setOpexpByMode(response.data.data));
        axios.get(`http://45.33.31.186/spending_by_mode/?mode=MB,CB,RB,TB,PB${props.params}&expense_type=RS,FC,OC`)
          .then(response => setCapexpByMode(response.data.data));

      // axios.get(`http://45.33.31.186/opexp_by_service/?mode=MB,CB,RB,TB,PB${props.params}&expense_type=VO,VM,NVM,GA`)
  ...
  return (
    <div className="expenses">
        ...
        <OpexpByMode chartData={opexpByMode} axisLabel={"2022 Dollars"}/>
        <br/>
        <h2>Capital Expense</h2>
        <OpexpByMode chartData={capexpByMode} axisLabel={"2022 Dollars"}/>
        <br/>
        <h2>Cost Per Passenger</h2>
        <PerformanceByMode chartData={costPerUptByMode} axisLabel={"2022 Dollars / Passenger"}/>
        <h2>Cost Per Passenger Mile Traveled</h2>
        <PerformanceByMode chartData={costPerPmtByMode} axisLabel={"2022 Dollars / Mile"}/>
        <br/>
    </div>
  );
}
