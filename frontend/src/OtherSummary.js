...
function BusSummary(props) {
  ...
  React.useEffect(() => {
    if (props.params) {
      ...
      axios.get(`http://localhost:8000/cost_per_upt_by_mode/?mode=FB,OT,AT,nan${props.params}`)
        .then(response => setCostPerUptByMode(response.data.data))
      axios.get(`http://localhost:8000/cost_per_pmt_by_mode/?mode=FB,OT,AT,nan${props.params}`)
  ...
  return (
    <div className="expenses">
        <></>
        <h2>Ridership</h2>
        <OtherPerformanceByMode chartData={uptByMode}/>
        <h2>Passenger Miles</h2>
        <OtherPerformanceByMode chartData={pmtByMode}/>
