    ...
    if (active && payload && payload.length) {
      return (
        <div className="custom-tooltip" style={{ backgroundColor: "white", fontSize: "16px", margin: "10px" }}>
          ...
          {payload[0].payload.upt_per_vrm > 0 && (
            <p className="label">{`Passengers per Vehicle Mile: ${payload[0].payload.upt_per_vrm}`}</p>
          )}
...
const UptPerVrm = ({ chartData }) => (
    ...
    <Legend />
    <Line dataKey="upt_per_vrm" name="Passengers per Vehicle Mile" fill="Black" stroke="Black" />
...
UptPerVrm.propTypes = {
    chartData: PropTypes.arrayOf(
      PropTypes.shape({
        ...
        upt_per_vrm: PropTypes.number
      })
    ).isRequired
  }

export default UptPerVrm