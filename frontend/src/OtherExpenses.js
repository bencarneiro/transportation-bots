...
function BusExpenses(props) {
  ...
  React.useEffect(() => {
    ...
    axios.get(`/spending_by_category/?mode=FB,OT,AT,nan${props.params}&expense_type=VO,VM,NVM,GA`)
      .then(response => setOpexpByCategory(response.data.data));
    axios.get(`/spending_by_category/?mode=FB,OT,AT,nan${props.params}&expense_type=RS,FC,OC`)
  ...
  return (
    <div className="expenses">
        ...
        <OtherOpexpByMode chartData={opexpByMode} axisLabel={"2022 Dollars"}/>
        <br/>
        <h2>Capital Expense By Mode</h2>
        <OtherOpexpByMode chartData={capexpByMode} axisLabel={"2022 Dollars"}/>
        <br/>
