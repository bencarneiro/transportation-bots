    ...
    if (active && payload && payload.length) {
      return (
        <div className="custom-tooltip" style={{ backgroundColor: "white", fontSize: "16px", margin: "10px" }}>
          ...
          {payload[0].payload.vrm_per_vrh > 0 && (
            <p className="label">{`Service Miles per Service Hour: ${payload[0].payload.vrm_per_vrh}`}</p>
          )}
...
const VrmPerVrh = ({ chartData }) => (
    ...
    <Legend />
    <Line dataKey="vrm_per_vrh" name="Vehicle Miles per Vehicle Hour" fill="Black" stroke="Black" />
...
VrmPerVrh.propTypes = {
    chartData: PropTypes.arrayOf(
      PropTypes.shape({
        ...
        vrm_per_vrh: PropTypes.number
      })
    ).isRequired
  }

export default VrmPerVrh