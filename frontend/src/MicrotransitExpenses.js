...
function BusExpenses(props) {
  ...
  React.useEffect(() => {
    if (props.params) {
    axios.get(`http://45.33.31.186/spending_by_budget/?mode=DR,DT,VP,JT${props.params}`)
      .then(response => setSpendingByBudget(response.data.data));
    axios.get(`http://45.33.31.186/spending_by_category/?mode=DR,DT,VP,JT${props.params}&expense_type=VO,VM,NVM,GA`)
      .then(response => setOpexpByCategory(response.data.data));
    axios.get(`http://45.33.31.186/spending_by_category/?mode=DR,DT,VP,JT${props.params}&expense_type=RS,FC,OC`)
      .then(response => setCapexpByCategory(response.data.data));
    axios.get(`http://45.33.31.186/spending_by_mode/?mode=DR,DT,VP,JT${props.params}&expense_type=VO,VM,NVM,GA`)
      .then(response => setOpexpByMode(response.data.data));
    axios.get(`http://45.33.31.186/spending_by_mode/?mode=DR,DT,VP,JT${props.params}&expense_type=RS,FC,OC`)
      .then(response => setCapexpByMode(response.data.data));
      axios.get(`http://45.33.31.186/opexp_by_service/?mode=DR,DT,VP,JT${props.params}&expense_type=VO,VM,NVM,GA`)
