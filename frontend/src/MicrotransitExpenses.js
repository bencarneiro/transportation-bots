...
function BusExpenses(props) {
  ...
  React.useEffect(() => {
    ...
    axios.get(`http://localhost:8000/spending_by_category/?mode=DR,DT,VP,JT${props.params}&expense_type=VO,VM,NVM,GA`)
      .then(response => setOpexpByCategory(response.data.data));
    axios.get(`http://localhost:8000/spending_by_category/?mode=DR,DT,VP,JT${props.params}&expense_type=RS,FC,OC`)
