    ...
    if (active && payload && payload.length) {
      return (
        <div className="custom-tooltip" style={{ backgroundColor: "white", fontSize: "16px", margin: "10px" }}>
          ...
          {payload[0].payload.pmt_per_vrh > 0 && (
            <p className="label">{`Passenger Miles per Vehicle Hour: ${payload[0].payload.pmt_per_vrh}`}</p>
          )}
...
const PmtPerVrh = ({ chartData }) => (
    ...
    <Legend />
    <Line dataKey="pmt_per_vrh" name="Passenger Miles per Vehicle Hour" fill="Black" stroke="Black" />
...
PmtPerVrh.propTypes = {
    chartData: PropTypes.arrayOf(
      PropTypes.shape({
        ...
        pmt_per_vrh: PropTypes.number
      })
    ).isRequired
  }

export default PmtPerVrh