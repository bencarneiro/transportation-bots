...
function BusExpenses(props) {
  ...
  React.useEffect(() => {
    if (props.params) {
    axios.get(`/spending_by_budget/?mode=FB${props.params}`)
      .then(response => setSpendingByBudget(response.data.data));
    axios.get(`/spending_by_category/?mode=FB${props.params}&expense_type=VO,VM,NVM,GA`)
      .then(response => setOpexpByCategory(response.data.data));
    axios.get(`/spending_by_category/?mode=FB${props.params}&expense_type=RS,FC,OC`)
      .then(response => setCapexpByCategory(response.data.data));
      axios.get(`/opexp_by_service/?mode=FB${props.params}&expense_type=VO,VM,NVM,GA`)
