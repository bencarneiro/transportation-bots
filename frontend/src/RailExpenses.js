...
function BusExpenses(props) {
  ...
  React.useEffect(() => {
    ...
    axios.get(`http://localhost:8000/spending_by_category/?mode=LR,CR,HR,YR,SR,OR,MG,AR,IP,CC${props.params}&expense_type=VO,VM,NVM,GA`)
      .then(response => setOpexpByCategory(response.data.data));
    axios.get(`http://localhost:8000/spending_by_category/?mode=LR,CR,HR,YR,SR,OR,MG,AR,IP,CC${props.params}&expense_type=RS,FC,OC`)
